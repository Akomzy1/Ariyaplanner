...
2. Route: structured replies first (DONE/PAID/HERE/1) → deterministic handlers. Otherwise → Haiku classifier: {task_update | vendor_quote | question | idea_request | contribution | suggestion | other} → tool-called extraction (Zod) → update DB → confirm in-thread ("Logged: caterer quote ₦8,000/head for 200 — that's mid-range for Lagos. Budget updated.").
