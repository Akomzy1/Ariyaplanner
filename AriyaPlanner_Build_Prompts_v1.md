...
**28 sequenced prompts · prototype-first build · Ariya Planner Ltd**
...
**P4 — Seed data.** Implement `program_templates` and `price_bands` seed migrations per `skills/cultural-programs` (13 templates as structured blocks — 12 drafted with `is_live=false` pending consultants; the Office Celebration template is culture-neutral with no ritual content and ships `is_live=true`; naming templates include the symbolic-items, officiant and aqiqah-ram blocks per the updated skill) and `skills/price-bands` (full Lagos CSV, Abuja ±10%). Seed the **tips library v1** (PRD F1c: curated entries keyed event_type × workstream × runway_stage × guest_band, celebration + memorial register variants — no free-generated wisdom) and **idea_assets v1** (placeholder rows for the owned/licensed reference image set with license metadata fields; real images swap in pre-launch) plus a small **aso-ebi palette dataset** (named colour combinations with hex values for the deterministic palette-card renderer). Seed one demo event ("Mummy's 70th", celebration) and one memorial demo ("Papa's farewell") for development. Acceptance: seeds idempotent; demo events render once UI exists.
...
**P6 — Marketing pages.** Homepage (5 event-type cards), event-type pages (weddings celebration / burials memorial / office professional-warm), pricing, price-index hub — implementing `01`, `02a`, `02b`, `02c`, `03a`, `03b` prototypes pixel-faithfully. Office page indexed for SEO but excluded from hero/nav emphasis per positioning. Price Index renders live from `price_bands` per the skill. Static/ISR. Acceptance: side-by-side match, mobile-first, memorial page passes the `skills/memorial-tone` checklist.

**P7 — Architect intake (UI).** The conversational intake from `04-architect.html`: chat surface with selectable chips (event type incl. Office celebration, culture, religion — both skipped for office events — city, guest band, budget band, dates), streaming responses, register switches live the moment burial is selected. State persisted to `events` draft. Acceptance: matches prototype including memorial variant.

**P8 — Architect agent (backend).** `lib/agents/architect.ts`: Sonnet-class, system prompt encoding CLAUDE.md principles, Zod tool schema `create_event_plan` (event fields + template selection + budget tier). Template retrieval per `skills/cultural-programs` rules incl. fallback + `template_gap` logging + the non-removable elders footer. **Advisory Layer (PRD F1c):** grounded reactive Q&A (answers use the event's guest count/budget tier/runway, never generic); a Zod-validated `suggest_ideas` tool returning structured ideas (title, est cost from `price_bands` or estimate-pending — never invented numbers, optional `idea_assets` refs) written to `event_ideas`; tips retrieved from the `tips` table only; ideas never modify vetted ritual blocks (suggest alongside); full memorial reshaping per `skills/memorial-tone` (no celebratory theming — order-of-service, tribute, programme-wording, MC-briefing advice instead). Acceptance: golden intake transcripts (Yoruba wedding, Christian burial, Muslim janazah) produce valid, correctly-templated plans; idea-generation golden test returns price-banded or estimate-pending ideas in both registers with zero invented prices.
...
**P10 — Runway scheduler.** Implement `/lib/scheduler` exactly per `skills/runway-scheduler`: topo sort, backward/forward pass, slack, critical path, feasibility, triage outputs, **sprint-tier outputs (≤14d daily digest payload), anticipated-date mode (do-now/on-trigger split, date-lock recompute)**, drift recompute. Acceptance: the golden tests in the skill pass, now including the naming birth-trigger test.

**P11 — Plan view + advisory surfaces.** The three-tab plan view from `04-architect.html`: PROGRAM (ceremony timeline from templates), BUDGET (line items, tiers, total vs band), TASKS (grouped by workstream, due dates from scheduler), runway indicator (comfortable/tight/triage states). **Advisory surfaces per prototype:** inline tip slots (tips ride the plan view — capped, never standalone), idea cards with `idea_assets` images and an "Add to plan" action that materialises via the F2a path (host/coordinator: direct; member: routed to approval), and the deterministic **aso-ebi palette card** component (rendered colour chips from the palette dataset — no generative imagery, per PRD v1.2 doctrine). Both registers. Acceptance: demo wedding + demo memorial match prototype incl. idea cards and palette card; "Add to plan" creates the correct budget line + task.
...
**P13 — Task engine + nudge scheduling (deterministic core).** Task CRUD, DONE flow, dispatcher (`lib/agents/dispatcher.ts`) computing who-gets-nudged-when from scheduler output + mode-aware cadence (memorial 1.5×). Nudges queue to `wa_messages` outbox (send infra in P17). **Sprint cadence (F1b):** runway ≤14d switches to daily morning digest to the Event Coordinator + same-day blocking-task chase; anticipated-date events expose the one-tap "Baby is here" date-lock action which triggers full replan + sprint start. **Tip slots (F1c):** nudge bodies may carry one contextual tip from the `tips` table matched on event_type × workstream × runway_stage — capped at one per message, register-correct, never a standalone tip message. In-app notification fallback. Acceptance: simulated clock tests produce correct nudge queue for both registers, incl. correct tip selection and the one-tip cap.
...
**P16 — Trial + Event Pass (Paystack).** 7-day trial starts at event creation; value-based gate: locking day-of command centre, sending vendor briefings, and PDF export require `pass_paid_at`. Paystack checkout (₦50,000, kobo), webhook verification, audit_log entries. Memorial guard: pass surface presented once, quietly, zero promo styling. Locked/gated states implement the trial-gate designs in the `05c`/`06` prototypes exactly. Acceptance: gate matrix tested (trial active/expired × paid/unpaid); webhook idempotent; locked states match prototype.

## PHASE 2 — WhatsApp + execution layer (Prompts 17–24)

**P17 — WA send infrastructure.** `lib/wa/`: Cloud API client, template + session senders, **session media sender (images from Supabase Storage — powers F1c idea images and palette cards in-thread; session-window only)**, window computation (`window.ts`, unit-tested per skill), outbox worker draining `wa_messages` with retries + rate limiting, STOP handling. Acceptance: window logic tests pass; outbox survives API failure with retry.
