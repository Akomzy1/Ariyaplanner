# AriyaPlanner — Product Requirements Document v1.6
...
- **Traditional coronation / chieftaincy & kingship events (deferred template family).** New-Oba/Obi/Emir installations, ofala, turbaning, and chieftaincy title conferment. **Not** covered by existing templates and deliberately not in launch scope: this is a deeper, more culturally-loaded event class than weddings — multi-day rites, partly-secret kingmaker components, ethnicity- and *town*-specific protocols (a Yoruba Oba installation ≠ an Igbo ofala ≠ a Northern turbaning), regalia, praise-singers, homage/protocol seating, dignitary-scale guest lists. Handled shallowly it insults rather than helps, damaging the core trust moat. Therefore a **Phase 3+ template family with its own specialist consultant sourcing** (often kingdom-specific, a heavier engagement than the wedding/burial consultants). Strong future fit — high-status, high-budget, heavily attended, and almost entirely untooled — consistent with the "urgent and uncontested Nigerian ceremonies" identity. Note: *church/convention "coronation" services* (parish crowning, convention finales, coronation-themed anniversaries) are already served by the birthday/milestone and office-celebration templates with light relabelling — no build needed; only the traditional/kingship meaning is deferred here.
